...
protobuf: protoc_host
	mkdir -p protobuf
	cd protobuf && cmake $(CMAKE_COMMON_ARGS) -DWITH_PROTOC=$(HOST_PROTOC_PATH)/protoc $(CMAKE_BASE)
	cd protobuf && cmake --build . $(GENERATOR_ARGS)

protoc_target:
	mkdir -p protoc
	cd protoc && cmake $(CMAKE_COMMON_ARGS) -Dprotobuf_BUILD_TESTS=OFF $(CMAKE_BASE)
	cd protoc && cmake --build . $(GENERATOR_ARGS)

protoc_host:
	mkdir -p $(HOST_PROTOC_PATH)
	cd $(HOST_PROTOC_PATH) && \
	cmake -DCMAKE_BUILD_TYPE=$(CMAKE_BUILD_TYPE) -Dprotobuf_BUILD_TESTS=OFF $(CMAKE_BASE) && \
